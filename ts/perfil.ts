  ...
  fetch(`${BASE_URL}/Clientes/${id_cliente}`, {
    ...
    .then((data) => {
      ...
      if (addresses.length > 0) {
        addresses
          .slice(0, 3)
          .forEach((direccion: { descripcion: string; provincia: string }) => {
            li.textContent =
  ...
  fetch(`${BASE_URL}/Ventas/cliente/${id_cliente}`, {
    ...
    .then((data) => {
      ...
      if (data.ventas?.length > 0) {
        data.ventas.forEach(
          (venta: { id: string; total: number; estado: string | null }) => {
            ...
            switch (venta.estado) {
              ...
              default:
                ...
                console.warn(
                  "No se ha podido pintar el estado del producto correctamente",
                );
                break;
            }
